...
async def run_consumer():
    ...
    q = await declare_consumer_queue(ch, "payment.events.queue", ["inventory.reserved"])
    ex, _ = await declare_topology(ch)

    async with q.iterator() as it:
        async for msg in it:
            async with msg.process(requeue=False):
                e = json.loads(msg.body)
                oid = e["aggregate_id"]
                corr = e["correlation_id"]
                amount = e["payload"]["amount"]
                provider_key = "pay-" + oid

                async with db.pool.acquire() as c:
                    async with c.transaction():
                        ins = await c.fetchval(
                        ...
                        await c.execute(
                            "INSERT INTO payments(order_id,amount,status,provider_key) "
                        ...
                        out = envelope(
                            "payment.failed" if failed else "payment.succeeded",
                            oid,
                            {"amount": amount, "provider_key": provider_key},
                            corr,
                        )
                        await c.execute(
                ...
                await ex.publish(
                    Message(
                        ...
                        message_id=out["event_id"],
                        correlation_id=corr,
                    ),
                    routing_key=out["event_type"],
                )
                async with db.pool.acquire() as c:
                    await c.execute(
