...
async def publish_pending(ch):
    ...
    async with db.pool.acquire() as c:
        ...
        for r in rows:
            await ex.publish(
                Message(
                    json.dumps(dict(r["payload"])).encode(),
                    delivery_mode=DeliveryMode.PERSISTENT,
                    message_id=r["event_id"],
        ...
        async with c.transaction():
            ...
            if t == "order.created":
                ...
                if ok:
                    ...
                    await c.execute(
                        "INSERT INTO reservations(order_id,items,status) "
                        "VALUES($1,$2::jsonb,'RESERVED') ON CONFLICT DO NOTHING",
                        oid,
                        json.dumps(items),
                    )
                    out = envelope("inventory.reserved", oid, event["payload"], corr)
                else:
                    out = envelope("inventory.rejected", oid, {"reason": "insufficient_stock"}, corr)
            elif t == "payment.failed":
                row = await c.fetchrow(
                    "SELECT items,status FROM reservations WHERE order_id=$1 FOR UPDATE",
                    oid,
                )
                if not row or row["status"] == "RELEASED":
                    return
                for i in row["items"]:
                    await c.execute(
                        "UPDATE stock SET available=available+$1 WHERE product_id=$2",
            ...
            await c.execute(
                "INSERT INTO outbox_events(event_id,event_type,aggregate_id,payload,correlation_id) "
        ...
        async with q.iterator() as it:
            async for msg in it:
                async with msg.process(requeue=False):
                    await handle(json.loads(msg.body))
                    await publish_pending(ch)
